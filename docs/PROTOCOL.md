...
| `cargo.scatter` | c→s | `{ x, y, cargo: {goodType: qty} }` — sent on own-ship destruction; server scatters the hold as shared cargo pods (≤5 units each, ±120u around the wreck, 90s expiry, 150-unit sanity cap) that ride `world.tick` drops and settle via the normal `drop.claim` race. Fire-and-forget. Offline path spawns equivalent local drops. |
