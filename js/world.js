...
function updateDrops(deltaTime) {
    ...
    for (let i = game.drops.length - 1; i >= 0; i--) {
        ...
        // Scoop by flying through (a running-silent hulk scoops nothing —
        // no interactions while dark; its own scattered pods sit at the
        // wreck until the crawl ends, if their fuse lasts that long)
        const dist = Math.sqrt(
            Math.pow(d.x - game.ship.x, 2) +
            Math.pow(d.y - game.ship.y, 2)
        );
        if (dist < 26 && !game.hulkState) {
